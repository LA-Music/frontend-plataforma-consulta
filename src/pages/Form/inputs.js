...
export const Fields = {
  ...
  email: function ({name, value, placeholder, dispatch}) {
  ...
  nomeCompleto: function({name, value, placeholder, dispatch}) {
    ...
    return (
      <Form.Group>
        <Form.Label className="">Seu nome completo:</Form.Label>
          ...
          // onChange={e => dispatch({type: 'ADD_FORM', payload: {...form, nome_produtor: e.target.value}})} 
  ...
  cpf: function({name, value, placeholder, dispatch}) {
  ...
  telefone: function ({name, value, placeholder, dispatch}) {
    const form = value
  ...
  nome: function({name, value, placeholder, dispatch}) {
    const form = value
  ...
  nomeArtistico: function ({name, value, placeholder, dispatch}) {
    const {state_form, form } = value
    ...
    return (
      <FieldSet className="w-100 mb-3" variant="filled">
          ...
          endAdornment={
            <InputAdornment position="end" >
              <IconButton
                ...
                edge="end"
                onClick={ () => { 
                  state_form[name] && dispatch({type: 'ADD_FORM', payload: {...form, [name]: [...form[name], state_form[name]]}}) 
        ...
        <div className="w-100 d-flex row mx-auto">
          {form.nome_artistico.map( (r, key) => (<TagLabel className="my-3 mr-3" key={key}>{r}<CloseTag 
          // onClick={e => removeArtista(r)}
           title="Remover">x</CloseTag></TagLabel>))}
        </div>
      </FieldSet>
    )
  },
    ...
    return (
      <FieldSet className="w-100 mb-3" variant="filled">
        <Form.Label className="">Redes sociais</Form.Label>
          ...
          />
          <div className="w-100 d-flex row mx-auto">
            {form.redes_sociais.map( (r, key) => (<TagLabel className="my-3 mr-3" key={key}>{r}<CloseTag 
            // onClick={e => removeSociais(r)} 
            title="Remover">x</CloseTag></TagLabel>))}
          </div>
      </FieldSet>
    )
  ...
  musicas: function({name, value, placeholder, dispatch}) {
    ...
    return (
      <FieldSet className="w-100 mb-3" variant="filled">
        ...
        />
        <div className="w-100 d-flex row mx-auto">
          {form[name].map( (r, key) => (<TagLabel className="my-3 mr-3" key={key}>{r} <CloseTag 
          // onClick={e => removeMusic(r)} 
          title="Remover">x</CloseTag></TagLabel>))}
        </div>
      </FieldSet>
    )
