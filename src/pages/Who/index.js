...
function Who(props) {
  ...
  return (
    <>
      ...
      <ButtonRadio onClick={() => dispatch({type: 'ADD_FORM', payload: {...data, who:'artist'}})} selected={data.who === 'artist'} className="d-block text-center w-100 text-capitalize py-3 my-3">
        <img src={data.who === 'artist' ? SongWhite :Song} alt="Song" className="mr-4 img-fluid" /> Artista
      </ButtonRadio>
      <ButtonRadio onClick={() => dispatch({type: 'ADD_FORM', payload: {...data, who:'producer'}})} selected={data.who === 'producer'} className="d-block text-center w-100 text-capitalize py-3 my-3">
